...
import {
  ...
  IWeather,
} from "./open-weather-geographic.remote-response"
...
export class OpenWeatherService {
  ...
  public async getByCoordinates(
    ...
    const res: IOpenWeatherForecast = await this.request.get<
      IOpenWeatherForecast
    >(url, urlParams)
  ...
  private convert(data: IOpenWeatherForecast): WeatherDatasDto {
    ...
    for (const item of data.list) {
      res.infos.push({
        conditionText: item.weather
          .map((x: IWeather): string => x.description)
          .join(" | "),
