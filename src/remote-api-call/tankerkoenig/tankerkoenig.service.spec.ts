...
describe("TankerkoenigService", () => {
  ...
  describe("Search for stations", () => {
    it("valid search", async () => {
      ...
      jest
        ...
        .mockImplementation(() => Promise.resolve(stationsResponse))
      // Act
      const stations: Array<TankerkoenigStationDto> = await service.searchStations(
        "apiKey",
        1,
        2,
      )
    ...
    it("make invalid request", async () => {
      ...
      jest
        .spyOn(requestService, "get")
        .mockImplementation(() => Promise.resolve(invalidResponse))
      // Act + Assert
      expect.assertions(1)
      try {
  ...
  describe("Search for prices", () => {
    it("valid search", async () => {
      ...
      jest
        .spyOn(requestService, "get")
        .mockImplementation(() => Promise.resolve(priceOpenStation))
      // Act
      const prices: Array<TankerkoenigPrice> = await service.getPrices(
        "apiKey",
        ["remoteId"],
      )
      // Assert
