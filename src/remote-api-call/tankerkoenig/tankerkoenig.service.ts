...
export class TankerkoenigService {
  ...
  public async searchStations (apiKey: string, latitude: number, longitude: number): Promise<Array<TankerkoenigStationDto>> {
    const url: string = `${this.baseUrl}/list.php`
    ...
    const res: ITankerkoenigListRemoteResponse | ITankerkoenigErrorRemoteResponse =
      await this.request.get<ITankerkoenigListRemoteResponse | ITankerkoenigErrorRemoteResponse>(url, urlParams)
    if (res.ok) {
  ...
  public async getPrices (apiKey: string, stationIds: Array<string>): Promise<Array<TankerkoenigPrice>> {
    ...
    const url: string = `${this.baseUrl}/prices.php`
    const urlParams: Array<string> = [
      `apikey=${apiKey}`,
      `ids=${stationIds.join(',')}`
    ]

    const res: ITankerkoenigPricesRemoteResponse | ITankerkoenigErrorRemoteResponse =
      await this.request.get<ITankerkoenigPricesRemoteResponse | ITankerkoenigErrorRemoteResponse>(url, urlParams)
    if (res.ok) {
