...
import { ISortOrderDto, ITimetableStationDto } from '@smartmirror/shared'
...
class StationList extends React.Component<IProps, IState>  {
  ...
  private async onEditDialogClose (name: DialogButtonName): Promise<void> {
    if (this.state.selectedStation && name === 'save') {
      ...
      const existingLocation: ITimetableStationDto = this.state.stations.find((x: ITimetableStationDto) => x.id === station.id)!
