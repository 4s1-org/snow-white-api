...
import { ICommonLocationDto, ISortOrderDto } from '@smartmirror/shared'
...
class LocationList extends React.Component<IProps, IState>  {
  ...
  private async onEditDialogClose(name: DialogButtonName): Promise<void> {
    if (this.state.selectedLocation && name === 'save') {
      ...
      const existingLocation: ICommonLocationDto = this.state.locations.find((x: ICommonLocationDto) => x.id === location.id)!
