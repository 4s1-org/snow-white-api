...
import { IFuelPricePricesDto } from '@smartmirror/shared'
...
class FuelPriceWidget extends React.Component<IProps, IState>  {
        ...
        <table className='table table-borderless table-sm'>
          <thead>
            <tr>
              <th>Tankstelle</th>
              {this.props.showE5 &&
                <th className='text-center' style={{ width: '50px' }}>E5</th>
              }
              {this.props.showE10 &&
                <th className='text-center' style={{ width: '50px' }}>E10</th>
              }
              {this.props.showDiesel &&
                <th className='text-center' style={{ width: '60px' }}>Diesel</th>
              }
            </tr>
          </thead>
          <tbody>
  ...
  private formatPrice (price: number | false): JSX.Element {
    ...
    return <div>{part0}<sup>{part1}</sup></div>
  }
