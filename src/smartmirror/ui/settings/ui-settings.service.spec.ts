...
describe("UiSettingsService", () => {
  ...
  beforeEach(async () => {
    ...
    trafficSettingsService = module.get<TrafficSettingsService>(
      TrafficSettingsService,
    )
    fuelPriceSettingsService = module.get<FuelPriceSettingsService>(
      FuelPriceSettingsService,
    )
    timetableSettingsService = module.get<TimetableSettingsService>(
      TimetableSettingsService,
    )
    weatherSettingsService = module.get<WeatherSettingsService>(
  ...
  it("load settings", async () => {
    ...
    jest
      .spyOn(timetableSettingsService, "getRecord")
      .mockResolvedValueOnce(timetableData)
    jest
      .spyOn(weatherSettingsService, "getRecord")
      .mockResolvedValueOnce(weatherData)
    jest
      .spyOn(trafficSettingsService, "getRecord")
      .mockResolvedValueOnce(trafficData)
    jest
      .spyOn(fuelPriceSettingsService, "getRecord")
      .mockResolvedValueOnce(fuelPriceData)
    // Act
