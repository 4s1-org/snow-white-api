...
export class UiFuelPriceService {
  ...
  public async getPrices(): Promise<Array<FuelPricePricesDto>> {
    ...
    const apiKey: string =
      settingsEntity.apiKey || process.env.APIKEY_TANKERKOENIG
    ...
    if (settingsEntity.isActive && apiKey) {
      ...
      const stationIds: Array<string> = stations.map(
        (station: FuelPriceStationEntity) => station.remoteId,
      )
      const prices: Array<TankerkoenigPrice> = await this.tankerkoenig.getPrices(
        apiKey,
        stationIds,
      )
      ...
      for (const station of stations) {
        const price: TankerkoenigPrice = prices.find(
          (x: TankerkoenigPrice) => x.remoteId === station.remoteId,
        )
        result.push({
          diesel: price.diesel,
          e10: price.e10,
      ...
      throw new BadRequestException(
        "Fuelprice could not be loaded due to incomplete settings.",
      )
    }
  }
}
