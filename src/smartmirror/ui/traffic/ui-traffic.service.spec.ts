...
import { UiTrafficService } from "./ui-traffic.service"
...
describe("UiTrafficService", () => {
  ...
  it("getRoute with no apikey", async () => {
    // Arrange
    const data: TrafficSettingsEntity = JSON.parse(
  ...
  it("switch direction VM", async () => {
    ...
    expect(res.text).toBe(
      `${someLocationFrom.name} nach ${someLocationTo.name}`,
    )
  })
  ...
  it("switch direction NM", async () => {
    ...
    expect(res).toBeDefined()
    expect(res.text).toBe(
      `${someLocationTo.name} nach ${someLocationFrom.name}`,
    )
  })
})
