...
export class UiTimetableService {
  ...
  public async getTimetable(): Promise<RmvTripsDto> {
    ...
    if (
      settingsEntity.isActive &&
      apiKey &&
      settingsEntity.timetableStationFrom &&
      settingsEntity.timetableStationTo
    ) {
      ...
      const routes: Array<RmvTripDto> = await this.rmv.getTrip(
        apiKey,
        stationFrom,
        stationTo,
        filter,
      ...
      throw new BadRequestException(
        "Timetable could not be loaded due to incomplete settings.",
      )
    }
  }
}
