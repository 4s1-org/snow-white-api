...
export class TimetableController {
  ...
  public async searchForStations(
    @Body() coordinates: CoordinatesDto,
  ): Promise<Array<RmvStationDto>> {
    const apiKey: string =
      (await this.settings.getRecord()).apiKey || process.env.APIKEY_RMV
    return this.stations.search(
      apiKey,
  ...
  @Get("/stations/:id")
  public loadSingleStation(
    @Param("id") id: string,
  ): Promise<TimetableStationDto> {
  ...
  public saveStation(
    @Param("id") id: string,
    @Body() station: TimetableStationDto,
  ): Promise<void> {
