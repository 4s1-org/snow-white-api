...
export class FuelPriceController {
  ...
  public async searchStations(
    @Body() latlon: CoordinatesDto,
  ): Promise<Array<TankerkoenigStationDto>> {
    const apiKey: string =
      (await this.settings.getRecord()).apiKey ||
      process.env.APIKEY_TANKERKOENIG
  ...
  @Get("/stations/:id")
  public async loadSingleGasStation(
    @Param("id") id: string,
  ): Promise<FuelPriceStationDto> {
  ...
  @Put("/stations/:id")
  public saveGasStation(
    @Param("id") id: string,
    @Body() body: FuelPriceStationDto,
  ): Promise<void> {
