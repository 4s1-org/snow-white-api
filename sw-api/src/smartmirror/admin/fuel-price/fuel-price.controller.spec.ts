...
describe('FuelPrice Controller', () => {
  ...
  beforeEach(async () => {
    ...
    stationsService = module.get<FuelPriceStationsService>(FuelPriceStationsService)
  ...
  describe('Stations', () => {
    it('POST /stations/search', async () => {
      ...
      jest.spyOn(stationsService, 'search').mockImplementation(() => Promise.resolve(resData))
      ...
      // Act
      const res: Array<TankerkoenigStationDto> = await controller.searchStations({ latitude: 50, longitude: 10 })
      // Assert
    ...
    it('GET /stations/:id', async () => {
      ...
      // Act
      const res: FuelPriceStationDto = await controller.loadSingleGasStation('foo')
      // Assert
    ...
    it('PUT /stations/reorder', async () => {
      // Arrange
      jest.spyOn(stationsService, 'reorderGasStations').mockImplementation(() => Promise.resolve())
      // Act
